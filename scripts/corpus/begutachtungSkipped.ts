...
const EXEMPT: Record<string, string> = {
  ...
  /* GP XXVII, curated 2026-09-16 by reading all 535 skipped titles. 27 items
   * carry a budget-procedure title, 21 of them are exempt. The other six stay
   * in the count with a reason — see below, right after the list. */
...
/* GP XXVII — the six budget titles that are NOT exempt, each with its reason.
 * They stand here and not in EXEMPT because not exempting something is just as
 * much a decision as exempting it and has to be just as readable.
 *
 * a) VORBELASTUNG PLUS A SUBSTANTIVE LAW — the authorisation is Budgetvollzug,
 *    the law carried along with it is not, and the substantive part could have
 *    gone to Begutachtung. Same line as with the Budgetsanierungsmaßnahmen-
 *    gesetz II in GP XXVIII: a title with „Budget" in it does not yet make a
 *    Budgetverfahren.
...
/**
 * Which Klubs were in government, and when? Needed to answer the question
 * every expert asks first: an Initiativantrag is parliamentary law — but whose?
 * A motion by the governing Klubs is a government project taking the short
 * road; an opposition motion is not.
 *
 * The table is hand-maintained like EXEMPT, and for the same reason: there is
 * no field for it. `frak_code` names the Klub, not its role — and the role
 * changes in the middle of a period. The date is when the motion arrived.
 *
 * Careful with XXVIII: from 24.10.2024 the ÖVP-Green government was only
 * caretaking AND without a majority in the House. „Nur Koalitionsklubs"
 * therefore does not mean the same inside that window as after it; which is
 * why the report separates by era instead of only summing.
 */
interface Era {
  from: string
  clubs: string[]
...
/** Which Klubs were in government on day `date`? */
...
/* Title comparison for the prior-history check (§3c). Deliberately coarse:
 * titles are short and the spelling wavers ("42.KFG-Novelle" against "42. KFG-
 * Novelle"). The calibration stands in the report — real ME→RV pairs are
 * measured by the same yardstick, so one can see what it is worth. */
...
const resolved: Resolved[] = await pool([...rvItems, ...antragItems], CONCURRENCY, async (item) => {
  ...
  // Who filed it? `names[]` carries the movers named individually with their
  // Klub code (`frak_code`). The "Kolleginnen und Kollegen" of the motion's
  // own wording are NOT in there — so "nur Koalitionsklubs" always means:
  // those named individually are.
  const clubs = [...new Set(
...
// ---------------------------------------------------------------------------
// 3b. Who files the Initiativanträge?
// ---------------------------------------------------------------------------

/* Without this line the number is open to attack, and rightly so:
 * "Initiativanträge sind Parlamentsrecht, das ist doch kein Befund." True —
 * as long as it is the MPs. Where it is the governing Klubs, it is a
 * government project skipping the draft stage. */

// How many Klubs does this House have? From the corpus rather than hard-coded.
...
// ---------------------------------------------------------------------------
// 3c. Prior history: was the content in Begutachtung as a Ministerialentwurf before?
// ---------------------------------------------------------------------------

/* Parliament's Rechts-, Legislativ- und Wissenschaftlicher Dienst recorded in
 * its Fachdossier of 31.10.2024 that legislative procedures begin as a
 * Ministerialentwurf and then arrive in the House as a selbständiger Antrag —
 * in the 2023/24 session for 19 of the 93 motions of ÖVP and the Greens. Such
 * laws WERE in Begutachtung, and until 16.09.2026 this script counted them as
 * skipped all the same. So: measure it.
 *
 * The structured route to it does not exist. A Ministerialentwurf's `stages`
 * do carry a successor pointer, but measured over GP XXVIII all 96 pointers
 * present point at a Regierungsvorlage and not one at a motion. The route
 * ME → Initiativantrag is invisible in the data — hence by title and date
 * here, and hence as a LIST OF CANDIDATES to read rather than a silent
 * re-booking.
 *
 * The finding is reported, not applied: the headline number stays and the
 * range stands beneath it. Same line as with the exemptions — whoever judges
 * the correction differently sees both numbers. */
...
// The title comes from the detail JSON, not from the list: list 81 carries
// the Betreff in column 4 and the RESSORT in column 6 — a mix-up that
// compares nonsense silently (happened once, 16.09.2026).
const meDetails = await pool(meRows, CONCURRENCY, async (me) => {
  ...
  return {
    ...
    frist,
    // Did the draft itself become a Regierungsvorlage? Then a motion of the
    // same name is more likely a second project on the same matter.
    rv: stages.flatMap((s) => [...String(s.text ?? '').matchAll(/\/gegenstand\/[IVXLC]+\/I\/(\d+)/g)].map((m) => m[1]!))[0] ?? null,
  }
})

/* Calibration: the same title measure over pairs that demonstrably belong
 * together (draft → its own Regierungsvorlage). It tells the reader what the
 * threshold below is worth instead of leaving them to guess. */
...
for (const a of antragKinds) {
  ...
  vorgeschichte.push({
    ...
    // Title identical AND the draft never became a Regierungsvorlage: then
    // the motion is the most plausible continuation.
    strong: best.score === 1 && !best.me.rv,
    // Motion filed before the deadline ran out — there was a Begutachtung,
    // but it was bypassed while still running.
...
/* The title match is the weak version of this check. The strong one compares
 * the Gesetzestexte and lives in `scripts/corpus/meAntragJoin.ts`; once it has
 * run, its result lies here and takes precedence. Separate scripts, because
 * the text match loads one document per Gegenstand and so has a completely
 * different runtime from the rest. */
...
// `strong` carries the correction, `schwach` only the range — the boundary is
// read off a gap in the data in the join script, not chosen.
...
// ---------------------------------------------------------------------------
// 3d. Is „ohne Begutachtung" the same as „ohne Öffentlichkeit"? No.
// ---------------------------------------------------------------------------

/* Since § 23b GOG-NR (in force 01.08.2021) Parliament publishes every
 * legislative initiative and admits Stellungnahmen to it — to motions and to
 * Regierungsvorlagen as well. A law that skipped the ministerial Begutachtung
 * has therefore not automatically been left without public participation.
 *
 * It is not the same procedure: no deadline, no invited circle, no ressort
 * that receives the Stellungnahmen and could work them in — but it is not
 * nothing either. Whoever says „nie öffentlich begutachtet" has to know this
 * number before saying it.
 *
 * List 142, `BEZUG_ITYP` = A (motions) or I (Regierungsvorlagen). The column
 * carrying the referenced Gegenstand is found by its header, not by an index
 * — see the column trap in §2 of the docs. */
/* TWO TRAPS, walked into once each on 16.09.2026:
 *
 * 1. `GP_CODE` is the period of the STELLUNGNAHME, `BEZUG_GP_CODE` that of
 *    the Gegenstand it refers to. For „Stellungnahmen zu den Gesetzen dieser
 *    Periode" only the second is right.
 *
 * 2. **The list is capped at 100.000 rows, even with `showAll=true`.**
 *    GP XXVII, BEZUG_ITYP=A: `count` reports 180.709, 100.000 are delivered.
 *    Whoever counts `rows` silently counts 55 % of the truth.
 *
 * So the question is asked per Gegenstand here and ONLY `count` is read —
 * without `showAll`, so a 20-row answer. That is exact, independent of the
 * cap, and costs one small request per law. The number can no longer depend
 * on a limit nobody sees in the result. */
...
if (vorgeschichte.length) {
  ...
  // The same correction applied to the government number above, or else two
  // numbers stand side by side of which only one is corrected.
...
await writeFile(rowsFile, JSON.stringify({
  ...
  totals: {
    ...
    // §3b: motions by the governing Klubs, and both together — the number
    // that answers the objection "Initiativanträge sind halt
    // Parlamentsrecht".
    koalitionsantraege: koaAntraege.length,
    regierungsvorhabenOhneBegutachtung: skippedRv.length + koaAntraege.length,
    // §3c: candidates with a Begutachtung prior history and the range they
    // open up. Reported, not applied.
    // Decisive as soon as corpus/meAntragJoin.ts has run.
    vorgeschichteBelegt: textJoin ? bestaetigt.length : null,
    skippedKorrigiertBelegt: textJoin ? korrBestaetigt : null,
    // The title-based earlier version; kept standing as a contrast.
