...
| `SpineRail` | `data: DraftDetail; anchors?; comparisonAnchors?; createsNewLaw?: boolean; amendedLawCount?: number; rvStatementTotal?: number\|null` | Where does this draft stand: five stations, one row each. It answers three things and nothing else — where the text is now, what happened at each station it passed, what is still ahead — and each station links into the section that carries it (§12.26) |
| `BgblOutcomeBlock` | `risId: string; outcome?: BgblOutcome\|null` | What became of a Verordnungsentwurf (§12.32). Three states, and two of them are not the same: up to 180 days after the Frist the line says it takes time, only after that that nothing can be found (`pnpm corpus:bgbl2`, median 57 days, p90 196) |
...
- `/entwuerfe/[id]` **Detail of a Begutachtung without a Gegenstand at Parliament** (§12.16): a RIS record, addressed by its `BEGUT_…` id. One link shape for every Entwurf since 18.09.2026 — the id shape decides which of the two detail pages renders, so a reader never has to know which half of the corpus a draft is in to guess its URL (§12.19). It deliberately has no station rail, no statements panel and no ME→RV comparison: none of them can exist here, because there is no Gegenstand to hang them on. What it does carry is the RIS documents, the Textgegenüberstellung where one exists, and `BgblOutcomeBlock` for what became of the Verordnung (§12.32).
