...
**⚠️ Lists do not share a column layout, and the collision is silent.** In list 81 the title is index **4** and index **6** is the ministry code; in list 101 index **6** is the title. Reading one list with the other's indices compares law titles against `BMJ`/`BMF` and returns a confident, clean zero — no error, no empty field, just a wrong answer that looks like a real one (paid for on 2026-09-16, `begutachtung-uebersprungen.md` §2). Two habits that would have caught it: take titles from the detail JSON rather than the row where the cost allows, and give every matching pass a control that is *supposed* to produce hits — a zero in the control is the alarm.
...
- **Filing open or closed:** `.content.statementsstate` is `"1"` while the item takes Stellungnahmen and `"0"` afterwards — on a Ministerialentwurf while the Frist runs, on a Regierungsvorlage while the Nationalrat has the text (verified 2026-09-15: 132/ME with running Frist `"1"`, the six newest GP-XXVIII Vorlagen `"1"`, the enacted 2238 d.B. and the closed 95/ME `"0"`). `.content.statements.new` mirrors it as 1/0. No deadline is published for the Vorlage; the form closes with the vote. The monitor shows the Vorlage's door from this flag (`enactment.filingOpen`), gated on the GP still running.
- **Process history:** `.content.stages[]` — arrival → deadline end → transmission → "Regierungsvorlage (`<a href="/gegenstand/XXVIII/I/474">474 d.B.</a>`)". The RV link is an href inside HTML text (regex-parseable). It is a second, independent path for the ME→RV join, which today runs off the Regierungsvorlage's `preconst`. **But it only ever names a Regierungsvorlage:** on GP XXVIII, 96 of 132 drafts carry a forward pointer and all 96 point at an `I`, none at an `A`. A draft that resurfaces as an Initiativantrag has no forward link at all, and its Antrag does not mention it either — that path has to be recovered from the text (`scripts/corpus/meAntragJoin.ts`, `begutachtung-uebersprungen.md` §4a).
- **Proposers with their club:** `.content.names[]` with `funktext: "Eingebracht von"` carries `frak_code` (`V`, `S`, `F`, `G`, `N`) on Anträge. Named proposers only — the "Kolleginnen und Kollegen" of the motion text are not enumerated, so any count is over the named ones.
- **`Gesetzestext` exists as HTML on every type** — ME, Antrag ("Gesetzestext (Arbeitsdokument ParlDion)") and Regierungsvorlage. Comparable across types, which is what makes a text join between them possible; Erläuterungen and Textgegenüberstellung differ in scope between the types and are not comparable.
- **How the clubs voted:** `.content.vote` on a **Regierungsvorlage** — `result[]` with one entry per Klub (`text`, `infavor`, plus `fraction` = its mandates and a `color`), `infavor` at the top level for whether the vote carried, and `text`/`comment` as upstream's own prose. `null` until the third reading. Where upstream has no club record it ships `result: []` and a word in `text` instead („Namentliche Abstimmung", „mehrstimmig", „Einstimmig"; `code` then starts with `_`). Coverage, the two shapes and what the field does NOT say: §101 below.
...
- **Measured over GP XXVI–XXVIII** (651 Ministerialentwürfe, `pnpm corpus:stationen`, 17.09.2026), because the station selector reads exactly this list:
  - **The three station titles above are the whole vocabulary** and stable across all three periods. Per period: 112/296/91 drafts carry an RV text, 40/80/40 an Ausschussfassung, 34/59/29 a Plenarfassung.
  - **Every one of them is published as HTML** — 154 Ausschuss- and 122 Plenarfassungen, not a single PDF-only case. PDF-only is a property of the *draft's own* text in the older periods (77 of 353 in GP XXVII), never of the later stations, and never of the period as such: 276 of those 353 GP-XXVII drafts do offer HTML. A note in `lawDiffService.ts` claimed the opposite ("GP XXVII and earlier are PDF-only") and was corrected.
  - **The list also carries documents that are not versions of the law text:** "Verhältnismäßigkeitsprüfung" (the EU assessment for regulated professions — 171/ME, 309/ME XXVII, 160/ME XXVI) and "Vertragstext" (a Staatsvertrag — 79/ME, 97/ME XXVI). Read as a station, a § parser turns an annex into paragraphs. Hence an exact-title whitelist, the same rule §1 already states for document names and shortinfo headings.
  - **The draft's own text is titled freely too:** "Gesetzestext" (647×), plus "Gesetzestext (korrigierte Version)" and "(ursprüngliche Version)" on one draft that was corrected mid-Begutachtung (315/ME XXVII), and — the trap — "Gesetzestext, Vorblatt und Erläuterungen" as ONE document on three GP-XXVI drafts. Those three have no isolated law text; a prefix match would compare explanatory prose against law text.
  - The Plenarfassung is filed under a `/BNR/` document path (Beschluss des Nationalrats), not under the Vorlage's `/I/` — irrelevant for reading it, but it means the URL prefix is not a reliable station signal either.
...
- Without `BEZUG_INR`: all ME Stellungnahmen of the GP (XXVIII: 5,505). Completely unfiltered: 633,416 (all GPs, all types). Careful: `{"GP_CODE":["XXVIII"]}` is a **different** dimension (GP of the SN itself, all types, 8,513 rows) — not interchangeable with `BEZUG_*`. This is easy to get wrong in a way that still returns plausible data: mixing `GP_CODE` with `BEZUG_ITYP` on 2026-09-16 produced 30 laws / 679 statements where `BEZUG_GP_CODE` gives **43 laws / 960**, a third of the result missing with nothing to indicate it.
- Row (0-based): `[2]` SNME-INR (→ detail URL), `[4]` date, `[6]` submitter as HTML `<a>` with the name, `[12]` endorsements (int, **approximation only** — list 305 is authoritative, a 5-vs-4 discrepancy was observed), `[15]` citation (`476/SN-88/ME`), `[18]` parent path, `[19]` **`TYP`: `I` for an institution, `P` for a person** (see §4).
- **`[0]` is the period of the STELLUNGNAHME, not of the draft, and the two part at every change of period.** A consultation running when the new Nationalrat convenes keeps collecting statements, and those rows carry the new GP in `[0]` while `BEZUG_GP_CODE` stays on the old one: XXVII 351/ME (Frist 05.11.2024) answers 17 rows, 14 `XXVII` and 3 `XXVIII` (measured 23.09.2026); 352/ME has 4 such rows, and over all 6,279 SNME rows of GP XXVIII exactly 7 cross the boundary. So the standard sanity check on a filtered list — every row must carry the requested GP in `[0]` — is **wrong on list 142** and returned a 502 for those two drafts; `[18]`, the parent path, is the column that asserts what `BEZUG_*` actually filtered (`server/utils/parliament/list142.ts`). Since the same day the check reads the filter dimensions themselves as well: `[7]`/`[8]`/`[9]` carry `feld_name` `BEZUG_GP_CODE`, `BEZUG_INR`, `BEZUG_ITYP` — strings on all 6,828 saved rows of GP XXVIII (GP `'XXVIII'`, INR without leading zeros, ITYP `'ME'`/`'I'`), never empty, and in 0 rows at odds with `[18]`. Link and columns must name the same parent; a disagreement between them is a failed filter too, and the header asserts all four.
- Document links are NOT in the row → fetch the SNME detail, one request per Stellungnahme. The monitor resolves only the rows that scroll into view, batched (`POST /api/stellungnahmen/dokumente`) and cached a week: 700 rows in the DOM cost the ten a reader looks at. Measured 2026-09-16 over 60 statements of 132/ME: **14 carry a PDF, 46 none** — a web-form submission has no file and its text sits inline in the detail JSON instead (`content.statement`), so "no document" is an answer, not a gap.
- **`BEZUG_ITYP` is not limited to `ME`.** `"I"` returns the Stellungnahmen to Regierungsvorlagen (shipped as "Stellungnahmen zur Regierungsvorlage"), and **`"A"` those to Initiativanträge** — the § 23b GOG-NR channel, in force since 01.08.2021, under which parliament publishes *every* Gesetzesinitiative and accepts statements on it for as long as the bill is before the house. GP XXVIII: 781 statements on Anträge, 466 of them on a single one (372/A, Waffengesetz-Novelle); GP XXVII: 96,064 on the COVID-19-Impfpflichtgesetz alone. **The monitor does not cover this channel today** — it is the largest known gap in the corpus (`begutachtung-uebersprungen.md` §4c for why it also constrains the wording of "ohne Begutachtung"). Find the referenced bill through the column whose header `label` is `Zu`, not by index.
...
- **And it is not limited to bills either.** `BEZUG_ITYP: "PET"` and `"BI"` return the statements on **Petitionen** and **parlamentarische Bürgerinitiativen**, which accept input and endorsements for as long as the item is before the house — no deadline at all, unlike a Begutachtungsfrist. GP XXVIII: 420 statements on 27 Petitionen, 792 on 36 Bürgerinitiativen. `BEZUG_ITYP: "VBG"` returns 0; Volksbegehren use a different route (`volksbegehren.md`). The outcome side is list 101 (`ITYP: "PET"|"BI"`) plus `status.description` on the Gegenstand detail, which spells the result in prose: GP XXVII ended 50 of 74 Bürgerinitiativen and 115 of 148 Petitionen in an `Ausschussbericht … zur Kenntnis genommen`, and a subject committee reported on only 3 and 4 of them respectively. Worth knowing for one structural reason: a Bürgerinitiative that does not conclude is carried into the next GP under § 21 Abs. 1a GOG **with an explicit successor link in `status.description`** (all 18 of GP XXVII's resolve to `XXVIII/BI/1`–`18`) — a real successor pointer of the kind the ME → Antrag route lacks entirely. Petitionen never carry over. Measured 2026-09-16; the reasoning for why none of this is a work package is `begutachtung-uebersprungen.md` §4c.
- **⚠️ `showAll=true` is capped at 100,000 rows, silently.** GP XXVII with `BEZUG_ITYP:["A"]` reports `count: 180709` and delivers `rows.length === 100000` — no error, no flag, no truncation marker. Counting `rows` there counts 55 % of the data and looks perfectly healthy (measured 2026-09-16; the largest list the project had touched before was 41k, which is why it went unnoticed). **Always compare `count` against `rows.length` after a `showAll` fetch.** Where only totals are needed, ask per parent item *without* `showAll` and read `count`: exact, cap-proof, and a 20-row response per request.
- **Header shape:** every entry carries `feldId`, `label` and — for the filterable dimensions only — `feld_name` (`GP_CODE`, `ITYP`, `INR`, `DATUM`, `DATUM_SORT`; the computed columns such as `Von`, `Unterstützungen`, `Nr` have a `label` only). One exception, and it matters: column 19 is hidden and its `label` is literally `"?"`, but it does carry `feld_name: "TYP"` — which is the only identity the header check can assert it by. The monitor asserts the positions it reads against this header on every fetch (`server/utils/parliament/listHeaders.ts`, lists 81 and 142): a reordered column used to degrade silently into "every submitter is a Privatperson", because that is the classifier's safe default.

**The same list holds the Stellungnahmen on Regierungsvorlagen** (found 2026-09-15 after a user pointed at 2238 d.B. — until then the monitor showed only the first round). `BEZUG_ITYP: ["I"]` with the RV's INR; the rows have item type `SN` instead of `SNME`, the citation is bare (`277139/SN`, no `-95/ME` tail), `Zu` reads `2238/I`, and the SN page is `/gegenstand/{GP}/SN/{inr}`. The RV detail JSON embeds the ready-made definition under `.content.statements.filter.data.definition`. Header identical to the ME case (same 23 columns). Sizes: GP XXVIII **555 on 68 Regierungsvorlagen**; GP XXVII **92,037 on 174**, dominated by the COVID-era Vorlagen (1289 d.B. alone: 41,376) — which is why the monitor sizes the list with the paging call above and fetches the breakdown only below a cap (`RV_STATEMENTS_CAP`, `parliament/statements.ts`). Odd but real: `[12]` Unterstützungen is populated on SN rows (4 on 277139/SN) although the RV detail's `approvalstate` says no endorsement is possible for the type. Shown as upstream counts it.
...
- Consequence for us: `getStatementsForMe` throws rather than caching a
  zero, and the persisted last-good store (`server/utils/parliament/lastgood.ts`)
...
- Evidence of the outage: a per-ME comparison of 2026-08-31, recorded
...
- **`VOTE` … `VOTE_COMMENT` (indices 31–35) carry the third-reading vote, club by club** (added here 2026-09-24). The header labels them „Abstimmung 3. Lesung", „Dafür", „Dagegen", „Abstimmungstext", „Abstimmungskommentar"; `VOTE` is `1`/`0` for carried/not, `VOTE_YES`/`VOTE_NO` are JSON arrays of club names (`["ÖVP","SPÖ","NEOS"]`), `VOTE_TEXT` upstream's own summary („Dafür: V, S, N, Dagegen: F, G"). The same record sits in each item's detail JSON under `content.vote` (§3), which is where the monitor reads it — no request of its own, since the RV detail is already fetched for the BGBl link.

  **Coverage, measured 2026-09-24 over nine periods** (Regierungsvorlagen, `ITYP=I`, `VHG=RV`): of the Vorlagen whose `Status` is `5`, **110 of 111** (GP XXVIII) and **361 of 362** (XXVII) carry a vote — the ones without were still in Behandlung. Club-level arrays are present on 108/110 (XXVIII), 360/361 (XXVII), 107/115 (XXVI), and still 398/423 in GP XX. The residue is upstream's *prose* vocabulary instead of the arrays: „Namentliche Abstimmung" (then `VOTE_COMMENT` has the Ja/Nein counts), „mehrstimmig", „Einstimmig". Nothing is parsed out of that comment — its format shifts between periods and once even misspells itself („abgegene Stimmen", GP XXV).

  **What the numbers say, and what they do not.** Only 32 % (XXVIII) / 15 % (XXVII) of Vorlagen passed on coalition votes alone; 26 % / 36 % had no club against them, and the rest carried opposition votes too — so the vote is not derivable from the government of the day. But it is **one vote on the whole bill at the end**, never per §: a club can vote against a law whose Abänderungsantrag it wrote. `VOTE_COMMENT` also shows why the seat counts in `content.vote.result[].fraction` must not be added up into a result — the namentliche Abstimmungen record 176 and 163 votes cast out of 183 mandates.

  The vote is a property of the **Regierungsvorlage**, not of the procedure: the 459 Ausschussberichte of GP XXVIII carry none, and neither do the BNR items; among Anträge only the few voted in plenary directly have one (64 of 1.047 in GP XXVIII).

  **Licence:** Parliament's own dataset page for Regierungsvorlagen puts „die Ergebnislisten der Filter und der API" and the Geschichtsseiten under CC BY 4.0, so this field is covered — it is a station *after* the Begutachtung, outside question E3.
...
**How often each document is actually there** — whole corpus, 4.574 records, 2026-09-17 (`pnpm corpus:verordnungen`, which runs the shipped mapper rather than a second copy of it):
...
Private persons are **fully identifiable on three levels** straight from the API: list 142 (last name, first name in the row), SNME detail (name **plus postal code and town**), list 305 (endorsers with name, postal code, town). Web-form full texts sit inline in the JSON; whether they fall under the CC-BY metadata license or under the full-text exclusion needs legal clarification — **the transport format does not decide the license question.** Our pipeline must enforce the metadata-only/no-names rules; the API does not help.

**There IS a structural organisation/person flag — corrected 2026-09-16.** This document said the opposite until then, and the classifier was built on name heuristics alone because of it. List 142 column 19 (`feld_name: "TYP"`, hidden, label `"?"`) holds `I` for an institution and `P` for a person. Measured over **106,626 rows, populated on 100 % of them**: GP XXVIII/ME 6,071 (I 2,822 / P 3,249), GP XXVII/ME 100,000 of 235,864 (I 3,739 / P 96,261), GP XXVIII Stellungnahmen zur RV 555 (I 395 / P 160). It agrees with `classifySubmitter` on 96.8 %, 99.7 % and 94.2 %.

What it encodes is **how the submitter registered, not what their name denotes** — which is why the two disagree in both directions, asymmetrically:

- **Flag `P`, name reads as an organisation** (5 / 24 / 0 rows). Hand-checked: roughly sixteen of the 29 name a real person — `Windland Energieerzeugungs GmbH; <Vorname Nachname>`, `i.A. <Nachname>, <Verband>`, `<Nachname> (für <AG>), <Vorname>`. **The name heuristic cannot reach this class**: the segment that names the submitter is org-shaped and the person stands behind it, so `audit/classifier`'s leak list looks straight past them. The flag never reads the name, so it sees them.
- **Flag `I`, name reads as a person** (176 / 287 / 30 rows). Mostly real institutions rendered as "Privatperson" — Datenschutzrat, KommAustria, Amnesty International, Austropapier, Naturhistorisches Museum Wien, pro mente OÖ. One instructive case: `Bezirksvorstehung Innere Stadt (1010 Wien)` is hidden because the PLZ-suffix rule reads `(1010 Wien)` as a private person's home town.

**How the monitor uses it (`classifySubmitter`, `server/utils/parliament/privacy.ts`): as a veto, never as authorisation.** `P` suppresses a name the string alone would have published; `I` changes nothing by itself, because publishing on an undocumented upstream column would hand it the hard invariant — one silent flip and the site republishes names. The `I` disagreements are printed as a review queue by `pnpm audit:classifier` and `ORG_ALLOWLIST` is what actually publishes one, by hand. The allowlist therefore outranks the flag, which is the escape hatch for the ~11 organisations per GP whose staff registered privately (`Bundestheater Holding GmbH, BTH`, `Patentanwaltskammer, Österr.`). Two further limits: the flag is **orthogonal to "nicht öffentlich"** (902 non-public rows are `P`, 13 are `I`), so the placeholder string stays the only source of truth there; and column 19 is asserted in `parliament/listHeaders.ts` by its `feld_name`, so a move fails loudly.

Old-vs-new corpus comparison on adoption (2026-09-16, all three corpora): **0 names newly published**, 5 + 24 + 0 rows no longer published.

**What the name heuristic gets wrong, measured (2026-09-15, `scripts/audit/classifier.ts` over all 6,067 GP-XXVIII rows; GP XXVII hits the 100,000-row cap and is COVID-skewed, so only per-item numbers are quoted for it):**
...
- **Persons published as organisations — the direction that matters:** `Lastname, Firstname; Universität Salzburg`, `Name, Dr.; Rechtsanwalt; … Rechtsanwälte GesbR`, `Name Name, Richterin am Landesgericht …` — a person filing with an affiliation, and the affiliation's keyword won. 239 rows in GP XXVII, 2 in GP XXVIII. Since 2026-09-15 the segment that names the submitter decides (`leadsWithPersonName` in `server/utils/parliament/privacy.ts`), checked before any organisation signal.
